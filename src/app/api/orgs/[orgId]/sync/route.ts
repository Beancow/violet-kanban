...
import { BoardList, BoardCard } from '@/types/appState.type';
...
export async function GET(request: NextRequest) {
    try {
        ...
        const [orgResult, boardsResult] = await Promise.all([
            getOrganizationServerAction(orgId, user.id),
            getBoardsForOrganizationServerAction(orgId),
        ]);

        if (!orgResult.success || !boardsResult.success) {
            const errorResult = [orgResult, boardsResult].find(
                (r) => !r.success
            );
