...
export function reducer(state: QueueState, action: Action): QueueState {
    return produce(state, (draft: Draft<QueueState>) => {
        switch (action.type) {
            case 'ENQUEUE_BOARD':
                draft.boardActionQueue = squashQueueActions(draft.boardActionQueue, action.action);
                return;
            case 'ENQUEUE_LIST':
                draft.listActionQueue = squashQueueActions(draft.listActionQueue, action.action);
                return;
            case 'ENQUEUE_CARD':
                draft.cardActionQueue = squashQueueActions(draft.cardActionQueue, action.action);
                return;
            case 'REMOVE_BOARD_BY_ID':
                draft.boardActionQueue = draft.boardActionQueue.filter((a) => getActionItemId(a) !== action.itemId);
                return;
            case 'REMOVE_LIST_BY_ID':
                draft.listActionQueue = draft.listActionQueue.filter((a) => getActionItemId(a) !== action.itemId);
                return;
            case 'REMOVE_CARD_BY_ID':
                draft.cardActionQueue = draft.cardActionQueue.filter((a) => getActionItemId(a) !== action.itemId);
                return;
            case 'SET_STATE':
                return action.state;
        }
